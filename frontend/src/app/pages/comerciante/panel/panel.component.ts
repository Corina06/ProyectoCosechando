...
export class PanelComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    // No cargar datos de reportes automáticamente - se cargarán según el usuario
  }
  ...
  loadDashboardData(): void {
    ...
    if (!userData) {
      console.log('⚠️ No hay datos de usuario, mostrando dashboard vacío');
      this.loadEmptyDashboard();
      this.loading = false;
      return;
    }
    ...
    try {
      ...
      const userContact = user.celular;
      
      if (!userContact) {
        console.log('⚠️ Usuario sin contacto, mostrando dashboard vacío');
        this.loadEmptyDashboard();
        this.loading = false;
        return;
      }

      console.log('🔍 Cargando dashboard para usuario con contacto:', userContact);
      
      // Cargar estadísticas del dashboard
      this.dashboardService.getDashboardStatsByContact(userContact).subscribe({
        next: (stats) => {
          console.log('📊 Estadísticas recibidas:', stats);
          this.dashboardStats = stats;
          
          // Inicializar datos de reportes vacíos para usuario nuevo
          this.initializeEmptyReports();
          this.loading = false;
        },
        error: (error) => {
      ...
      this.dashboardService.getMerchantOrdersByContact(userContact).subscribe({
        next: (orders) => {
  ...
  loadReportsData(): void {
    ...
    // Por ahora no cargar datos - se cargarán según el usuario
    console.log('📊 loadReportsData llamado - no cargando datos de ejemplo');
  }

  initializeEmptyReports(): void {
    // Inicializar datos de reportes vacíos
    console.log('📊 Inicializando reportes vacíos para usuario nuevo');
    
    this.productosMasVendidos = [];
    this.ventasPorPeriodo = [
      { periodo: 'Hoy', ventas: 0, ordenes: 0, crecimiento: 0 },
      { periodo: 'Esta Semana', ventas: 0, ordenes: 0, crecimiento: 0 },
      { periodo: 'Este Mes', ventas: 0, ordenes: 0, crecimiento: 0 },
      { periodo: 'Último Mes', ventas: 0, ordenes: 0, crecimiento: 0 }
    ];
  ...
  loadEmptyDashboard(): void {
    // Dashboard vacío para usuarios nuevos
    console.log('📊 Cargando dashboard vacío para usuario nuevo');
