...
export class PerfilComponent implements OnInit, OnDestroy {
  ...
  loadFallbackProfile(): void {
    ...
    // Si no existe 'user', intentar con 'userData'
    if (!userData) {
    ...
    if (userData) {
      try {
        ...
        console.log('👤 Cargando perfil desde localStorage:', user);
        
        // Crear perfil desde los datos REALES del usuario
        this.comerciante = {
          id: user.id || Date.now(),
          nombre: user.name || '',
          ...
          numeroCuenta: user.cuenta ? user.cuenta.toString() : '',
          foto: '/assets/images/default-user.jpg',
          fechaRegistro: new Date().toLocaleDateString('es-ES'),
          estado: 'Activo',
          verificado: false, // Usuario nuevo no verificado
          totalVentas: 0, // Usuario nuevo sin ventas
          productosActivos: 0 // Usuario nuevo sin productos
        };
        
        console.log('✅ Perfil creado desde datos reales:', this.comerciante);
      } catch (error) {
        console.error('❌ Error parsing user data:', error);
        this.createEmptyProfile();
      }
    } else {
      console.log('⚠️ No hay datos de usuario, creando perfil vacío');
      this.createEmptyProfile();
    }
  ...
  // Método para crear un perfil vacío para usuarios nuevos
  private createEmptyProfile(): void {
    this.comerciante = {
      id: Date.now(),
      nombre: '',
      apellido: '',
