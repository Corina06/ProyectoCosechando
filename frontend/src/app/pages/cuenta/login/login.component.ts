...
export class LoginComponent {
  ...
  onLogin() {
    ...
    if (!this.email.trim()) {
      this.errorMessage = '⚠️ Por favor ingresa tu correo electrónico';
      return;
    }

    if (!this.password.trim()) {
      this.errorMessage = '⚠️ Por favor ingresa tu contraseña';
      return;
    }
    ...
    this.authService.login(this.email, this.password).subscribe({
       ...
       error: (error) => {
         ...
         if (error.status === 400) {
           this.errorMessage = '❌ Correo electrónico o contraseña incorrectos. Verifica tus datos e intenta nuevamente.';
         } else if (error.status === 404) {
           this.errorMessage = '❌ No existe una cuenta con este correo electrónico. ¿Necesitas registrarte?';
         } else if (error.status === 0) {
           this.errorMessage = '❌ Error de conexión. Verifica tu internet e intenta nuevamente.';
         } else {
           this.errorMessage = '❌ Error al iniciar sesión. Por favor intenta nuevamente en unos momentos.';
         }
       }
     });
  }
